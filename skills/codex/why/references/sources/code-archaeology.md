...
Repository history ties evidence directly to the code that remains. It is still incomplete: squash merges can flatten intermediate reasoning, force-pushes can remove commits, hosted review discussion may be unavailable, and some approvals happen outside the repository.
...
When a substantive commit maps to a PR and authenticated read-only hosted access is available, pull the PR context with `gh` or an equivalent tool:

```bash
# Find the PR number from the merge commit or branch
git log -1 --format=%B <hash>

# Example using GitHub CLI for the PR body and review discussion
...
Look for in-repository rationale:
